...
{
    int bx = -1, by = -1, color = -1, dir = -1, nx, ny, outres;
    double exterior, l_um, w_um;
    string zname;

    try {
        cxxopts::Options options("bem3d", "BEM Simulation 3D");
        ...
        options.add_options()(
            ...
            "l,length", "Length in micrometers", cxxopts::value<double>())(
            "m,nx", "Number of blocks in x direction", cxxopts::value<int>())(
            "n,ny", "Number of blocks in y direction", cxxopts::value<int>())(
            "o,outres", "Output resolution", cxxopts::value<int>())(
            "w,width", "Width in micrometers", cxxopts::value<double>())(
            "z,zname", "Surface name", cxxopts::value<string>())(
            "h,help", "Print usage");
    ...
    // Extract height field data, required incident directions, and simulated
    // wavelengths
    ...
    if (bx == -1 || by == -1) {
        ...
        z = readData(
            "data/" + zname + "/patch" + to_string(bx) + to_string(by) +
            "/zvals.txt");
        ...
        if (color == -1) {
            printf(
                "You did not specify the considered wavelength for this "
                "subregion "
                "simulation. ");
            printf(
                "Please provide a integer between 0 and %d, inclusive. ",
                int(wvl.rows() - 1));
            printf("Only simulating the first wavelength.\n");
        }
        if (dir == -1) {
            printf(
                "You did not specify the considered incident direction for "
                "this "
                "subregion simulation. ");
            printf(
                "Please provide a integer between 0 and %d, inclusive. ",
                int(wi.rows() - 1));
    ...
    // Estimate the memory usage in each simulation and select a computation
    // plan
    ...
    int deviceCount, plan, ind1 = -1, ind40 = -1, ind41 = -1, ind42 = -1,
                           ind43 = -1;
    ...
    if (plan == 0)
        printf(
            "Performing simulations on the CPU because there is no GPU "
            "detected "
            "or GPU memory is insufficient for your simulation size.\n");
    ...
    // Perform simulations for the required wavelengths and illumination
    // directions
    ...
    for (int i = c1; i < c2; i++) {
        ...
        for (int j = d1; j < d2; j++) {
            ...
            if (bx == -1 || by == -1) {
                ...
                scatter->computeBRDF(
                    exterior, lambda, x1, outres, (float)(inci->irradiance1));
                averageBRDF = averageBRDF + 0.5f * scatter->brdf;
                scatter->computeBRDF(
                    exterior, lambda, x2, outres, (float)(inci->irradiance2));
                averageBRDF = averageBRDF + 0.5f * scatter->brdf;
                writeData(
                    "data/" + zname + "/BRDF_wvl" + to_string(i) + "_wi" +
                        to_string(j) + ".binary",
                    averageBRDF);
            }
            else {
                // For subregion simulations, only one wavelength and direction
                // is simulated at a time.
                ...
                scatter->computeFields(
                    exterior, lambda, x1, outres, xshift, yshift);
                ...
                EH_real = EH.real();
                EH_imag = EH.imag();
                // Scattered fields written as intermediate results. Will be
                // post-processed to synthesize BRDFs and then overwritten.
                writeBinary(
                    "data/" + zname + "/patch" + to_string(bx) + to_string(by) +
                        "/EH1_real.binary",
                    EH_real);
                writeBinary(
                    ...
                    EH_imag);
                scatter->computeFields(
                    exterior, lambda, x2, outres, xshift, yshift);
                EH.block(0 * outres, 0 * outres, outres, outres) = scatter->Ex;
                EH.block(0 * outres, 1 * outres, outres, outres) = scatter->Ey;
                EH.block(0 * outres, 2 * outres, outres, outres) = scatter->Ez;
                EH.block(1 * outres, 0 * outres, outres, outres) = scatter->Hx;
